/**
 * Keep class sets up the Miner gameObject which represents the Keep tower.
 * 
 * Mainly deals with the cannonballs that the tower throws at the closest enemies.
...
public class Keep extends Tower
{
	 ...
	 * @param y
	 */
	public Keep (GameState state, int x, int y)
	{
		this.state = state;
		this.x = x;
		this.y = y;
	...
	public void update(double timeElapsed)
	{
		...
		if (timeSinceLastShot < 1.5) //If it has been less than 1.5 seconds from the last shot, abort.
			return; 
		...
		EffectCannonball s = new EffectCannonball (state, towerPoint, e.getLocation());

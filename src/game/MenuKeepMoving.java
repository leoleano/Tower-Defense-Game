/**
 * MenuKeepMoving is called when a user clicks on a Keep in the Menu and has sufficient funds. 
 * At that point, an image of Keep is drawn wherever the mouse is until it's placed in a valid spot
 * in the game area.
 *
...
public class MenuKeepMoving extends Tower
{
	//Fields
	 ...
	 * @param state
	 */
	public MenuKeepMoving (GameState state)
	{
		this.state = state;
	...
	public void update (double timeElapsed)
	{
		...
		//MenuKeepMoving is removed from the game and adds a Keep object to the game.
		if (state.isMouseClicked() && p.distanceToNearestPathNode(q) > 30 && state.getMouseX() < 600)
		{
			state.removeGameObject(this);
			state.addGameObject(new Keep(state, state.getMouseX(), state.getMouseY()));
		}
	...
	{
		v.drawCenteredImage(g, "keep.png", state.getMouseX(), state.getMouseY(), 1.5);
